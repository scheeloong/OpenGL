...
public:
    ...
    enum {
        ROOT_TRANSLATE_X, ROOT_TRANSLATE_Y, ROOT_TRANSLATE_Z,
        ROOT_ROTATE_X, ROOT_ROTATE_Y, ROOT_ROTATE_Z,
        HEAD_PITCH, HEAD_YAW, HEAD_ROLL,
        R_SHOULDER_PITCH, R_SHOULDER_YAW, R_SHOULDER_ROLL, R_ELBOW, R_ELBOW_SCALE,
        L_SHOULDER_PITCH, L_SHOULDER_YAW, L_SHOULDER_ROLL, L_ELBOW, L_ELBOW_SCALE,
        R_HIP_PITCH, R_HIP_YAW, R_HIP_ROLL, R_KNEE,HEAD, BEAK, USELESS, BEAK_USElESS,
        L_HIP_PITCH, L_HIP_YAW, L_HIP_ROLL, L_KNEE,
        NUM_JOINT_ENUM
    };
    ...
    Keyframe() : id(0), time(0.0), jointDOFS(NUM_JOINT_ENUM) {
        setDOF(L_ELBOW_SCALE, 1.0);
        setDOF(R_ELBOW_SCALE, 1.0);
    }
    ...
    float getDOF(int eDOF) const {
        return jointDOFS[eDOF];
    }

    void setDOF(int eDOF, float val) {
        jointDOFS[eDOF] = val;
    }
    ...
    Vector getDOFVector() const {
        return jointDOFS;
    }

    void setDOFVector(const Vector& vec) {
        jointDOFS = vec;
    }
